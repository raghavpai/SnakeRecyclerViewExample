 ...
 */
public class SnakeGridLayoutManager extends RecyclerView.LayoutManager {
    ...
    public SnakeGridLayoutManager(GridSpanLookup spanLookup, int columns, float cellAspectRatio) {
    ...
    public SnakeGridLayoutManager(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.SnakeGridLayoutManager, defStyleAttr, defStyleRes);
        columns = a.getInt(R.styleable.SnakeGridLayoutManager_spanCount, 1);
        parseAspectRatio(a.getString(R.styleable.SnakeGridLayoutManager_aspectRatio));
        int orientation = a.getInt(R.styleable.SnakeGridLayoutManager_android_orientation, RecyclerView.VERTICAL);
        a.recycle();
        setAutoMeasureEnabled(true);
    }
    ...
    private int layoutRow(int rowIndex, int startTop, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        for (int position = firstPositionInRow;
            ...
            if (cell.row != 0 && cell.row % 2 == 0) {
                // for snake formation
                left = cellBorders[columns - cell.column - 1] + lp.leftMargin;
            } else {
                left = cellBorders[cell.column] + lp.leftMargin;
            }
            top = startTop + (cell.row * cellHeight) + lp.topMargin;
